...
pub fn distance(from: (f32, f32), to: (f32, f32)) -> u32 {
    ...
    let intermediate_term = 0.5 - (to.0 as f64 - from.0 as f64).to_radians().cos() / 2.
        + (from.0 as f64).to_radians().cos()
            * (to.0 as f64).to_radians().cos()
            * (1. - (to.1 as f64 - from.1 as f64).to_radians().cos())
            / 2.;

    const EARTH_RADIUS: u32 = 6371_000;
    /// 6371 km or 6371000 metres
...
#[cfg(test)]
mod tests {
    use crate::{angle_difference, angle_difference_deg, distance, heading, heading_deg};
    ...
    fn angle_difference_rad_test() {
        ...
        assert_approx_eq!(
            angle_difference(10f32.to_radians(), 370f32.to_radians()),
            0.
        );
        assert_approx_eq!(
            angle_difference(-50f32.to_radians(), 50f32.to_radians()),
            -100f32.to_radians()
        );
        assert_approx_eq!(
            angle_difference(-90f32.to_radians(), 91f32.to_radians()),
            179f32.to_radians()
        );
    }
    ...
    fn distance_test() {
        ...
        assert_eq!(
            distance((48.0708536, -1.7329389), (48.7322773, 2.3540089)),
            310_503
        );
        // Distance between Telecom Paris and the Eiffel tower
        assert_eq!(
            distance((48.713202, 2.200641), (48.858404, 2.294534)),
            17_550
        );
    }
    ...
    fn heading_test() {
        ...
        let lfrn = (
            (48f32 + 4.40 / 60.).to_radians(),
            (-(1f32 + 44.02 / 60.)).to_radians(),
        );
        let lfrd = (
            (48f32 + 35.50 / 60.).to_radians(),
            (-(2f32 + 4.85 / 60.)).to_radians(),
        );
