    ...
    I2C: WriteRead<Error = E> + Write<Error = E>,
{
    ...
    pub fn new(i2c: I2C) -> Self {
        Bno055 {
            ...
            mode: OperationMode::CONFIG_MODE,
            use_default_addr: true,
        }
    }

    /// Enables use of alternative I2C address `registers::BNO055_ALTERNATE_ADDR`.
    ...
    pub fn init(
        ...
        if id != registers::BNO055_ID {
            ...
            self.write_u8(registers::BNO055_SYS_TRIGGER, 0x00)
                .map_err(Error::I2c)?;

            if let Some(calibration_profile) = calibration_profile {
                self.set_calibration_profile(calibration_profile, delay)?;
            }

            self.set_mode(operation_mode, delay)?;
            delay.delay_ms(400);
    ...
    pub fn soft_reset(&mut self, delay: &mut dyn DelayMs<u16>) -> Result<(), Error<E>> {
        ...
        self.write_u8(
            registers::BNO055_SYS_TRIGGER,
            registers::BNO055_SYS_TRIGGER_RST_SYS_BIT,
        )
        .map_err(Error::I2c)?;
    ...
    /// Sets the operating mode, see [OperationMode](enum.OperationMode.html).
    /// See section 3.3.
    pub fn set_mode(
        &mut self,
        mode: OperationMode,
        ...
        if self.mode != mode {
            ...
            self.write_u8(registers::BNO055_OPR_MODE, mode.bits())
                .map_err(Error::I2c)?;
    ...
    /// Sets the power mode, see [PowerMode](enum.PowerMode.html)
    /// See section 3.2
    pub fn set_power_mode(&mut self, mode: PowerMode) -> Result<(), Error<E>> {
        self.set_page(RegisterPage::PAGE_0)?;

        self.write_u8(registers::BNO055_PWR_MODE, mode.bits())
            .map_err(Error::I2c)?;
    ...
    pub fn power_mode(&mut self) -> Result<PowerMode, Error<E>> {
        ...
        let mode = self
            .read_u8(registers::BNO055_PWR_MODE)
            .map_err(Error::I2c)?;

        Ok(PowerMode::from_bits_truncate(mode))
    }
    ...
    pub fn set_external_crystal(
        ...
        let prev = self.mode;
        self.set_mode(OperationMode::CONFIG_MODE, delay)?;
        self.write_u8(registers::BNO055_SYS_TRIGGER, if ext { 0x80 } else { 0x00 })
            .map_err(Error::I2c)?;
    ...
    pub fn set_axis_remap(&mut self, remap: AxisRemap) -> Result<(), Error<E>> {
        ...
        self.write_u8(registers::BNO055_AXIS_MAP_CONFIG, remap_value)
            .map_err(Error::I2c)?;
    ...
    pub fn axis_remap(&mut self) -> Result<AxisRemap, Error<E>> {
        ...
        let value = self
            .read_u8(registers::BNO055_AXIS_MAP_CONFIG)
            .map_err(Error::I2c)?;

        let remap = AxisRemap {
            x: AxisConfig::from_bits_truncate(value & 0b11),
            y: AxisConfig::from_bits_truncate((value >> 2) & 0b11),
            z: AxisConfig::from_bits_truncate((value >> 4) & 0b11),
        };
    ...
    pub fn set_axis_sign(&mut self, sign: AxisSign) -> Result<(), Error<E>> {
        ...
        self.write_u8(registers::BNO055_AXIS_MAP_SIGN, sign.bits())
            .map_err(Error::I2c)?;
    ...
    pub fn axis_sign(&mut self) -> Result<AxisSign, Error<E>> {
        ...
        let value = self
            .read_u8(registers::BNO055_AXIS_MAP_SIGN)
            .map_err(Error::I2c)?;
    ...
    pub fn get_revision(&mut self) -> Result<BNO055Revision, Error<E>> {
        ...
        self.read_bytes(registers::BNO055_ACC_ID, &mut buf)
            .map_err(Error::I2c)?;
    ...
    pub fn get_system_status(
        ...
        let self_test = if do_self_test {
            ...
            let sys_trigger = self
                .read_u8(registers::BNO055_SYS_TRIGGER)
                .map_err(Error::I2c)?;

            self.write_u8(registers::BNO055_SYS_TRIGGER, sys_trigger | 0x1)
                .map_err(Error::I2c)?;
            ...
            let result = self
                .read_u8(registers::BNO055_ST_RESULT)
                .map_err(Error::I2c)?;
        ...
        let status = self
            .read_u8(registers::BNO055_SYS_STATUS)
            .map_err(Error::I2c)?;
        let error = self
            .read_u8(registers::BNO055_SYS_ERR)
            .map_err(Error::I2c)?;
    ...
    pub fn quaternion(&mut self) -> Result<mint::Quaternion<f32>, Error<E>> {
        ...
        if self.is_in_fusion_mode()? {
            ...
            self.read_bytes(registers::BNO055_QUA_DATA_W_LSB, &mut buf)
                .map_err(Error::I2c)?;
            ...
            const SCALE: f32 = 1.0 / ((1 << 14) as f32);
    ...
    pub fn euler_angles(&mut self) -> Result<mint::EulerAngles<f32, ()>, Error<E>> {
        ...
        if self.is_in_fusion_mode()? {
            ...
            self.read_bytes(registers::BNO055_EUL_HEADING_LSB, &mut buf)
                .map_err(Error::I2c)?;
            ...
            const SCALE: f32 = 1f32 / 16f32; // 1 degree = 16 LSB
    ...
    pub fn get_calibration_status(&mut self) -> Result<CalibrationStatus, Error<E>> {
        ...
        let status = self
            .read_u8(registers::BNO055_CALIB_STAT)
            .map_err(Error::I2c)?;
    ...
    pub fn is_fully_calibrated(&mut self) -> Result<bool, Error<E>> {
        ...
        Ok(status.mag == 3 && status.gyr == 3 && status.acc == 3 && status.all == 3)
    }
    ...
    pub fn calibration_profile(
        ...
        self.read_bytes(registers::BNO055_ACC_OFFSET_X_LSB, &mut buf[..])
            .map_err(Error::I2c)?;

        let res = CalibrationProfile::from_buf(&buf);
    ...
    pub fn set_calibration_profile(
        ...
        let buf_reg = [registers::BNO055_ACC_OFFSET_X_LSB; 1];
    ...
    pub fn id(&mut self) -> Result<u8, Error<E>> {
        self.set_page(RegisterPage::PAGE_0)?;
        self.read_u8(registers::BNO055_CHIP_ID).map_err(Error::I2c)
    }

    /// Returns device's operation mode.
    pub fn get_mode(&mut self) -> Result<OperationMode, Error<E>> {
        self.set_page(RegisterPage::PAGE_0)?;

        let mode = self
            .read_u8(registers::BNO055_OPR_MODE)
            .map_err(Error::I2c)?;
        let mode = OperationMode::from_bits_truncate(mode);
        self.mode = mode;
    ...
    pub fn is_in_fusion_mode(&mut self) -> Result<bool, Error<E>> {
        let is_in_fusion = match self.mode {
            ...
            | OperationMode::M4G
            | OperationMode::NDOF_FMC_OFF
            | OperationMode::NDOF => true,
    ...
    fn set_page(&mut self, page: RegisterPage) -> Result<(), Error<E>> {
        if self.page != page {
            self.write_u8(registers::BNO055_PAGE_ID, page.bits())
                .map_err(Error::I2c)?;
        }
    ...
    pub fn linear_acceleration(&mut self) -> Result<mint::Vector3<f32>, Error<E>> {
        if self.is_in_fusion_mode()? {
            ...
            self.read_vec(registers::BNO055_LIA_DATA_X_LSB, SCALE)
        } else {
            Err(Error::InvalidMode)
        }
    }
    ...
    pub fn gravity(&mut self) -> Result<mint::Vector3<f32>, Error<E>> {
        if self.is_in_fusion_mode()? {
            self.set_page(RegisterPage::PAGE_0)?;
            const SCALE: f32 = 1. / 100.; // 1 m/s^2 = 100 lsb
            self.read_vec(registers::BNO055_GRV_DATA_X_LSB, SCALE)
        } else {
            Err(Error::InvalidMode)
        }
    }
    ...
    pub fn accel_data(&mut self) -> Result<mint::Vector3<f32>, Error<E>> {
        match self.mode {
            OperationMode::ACC_ONLY
            | OperationMode::ACC_GYRO
            | OperationMode::ACC_MAG
            | OperationMode::AMG => {
                self.set_page(RegisterPage::PAGE_0)?;
                const SCALE: f32 = 1. / 100.; // 1 m/s^2 = 100 lsb
                self.read_vec(registers::BNO055_ACC_DATA_X_LSB, SCALE)
            }
    ...
    pub fn gyro_data(&mut self) -> Result<mint::Vector3<f32>, Error<E>> {
        match self.mode {
            OperationMode::GYRO_ONLY
            | OperationMode::ACC_GYRO
            | OperationMode::MAG_GYRO
            | OperationMode::AMG => {
                self.set_page(RegisterPage::PAGE_0)?;
                const SCALE: f32 = 1. / 16.; // 1 deg/s = 16 lsb
                self.read_vec(registers::BNO055_GYR_DATA_X_LSB, SCALE)
            }
    ...
    pub fn mag_data(&mut self) -> Result<mint::Vector3<f32>, Error<E>> {
        match self.mode {
            OperationMode::MAG_ONLY
            | OperationMode::ACC_MAG
            | OperationMode::MAG_GYRO
            | OperationMode::AMG => {
                self.set_page(RegisterPage::PAGE_0)?;
                const SCALE: f32 = 1. / 16.; // 1 uT = 16 lsb
                self.read_vec(registers::BNO055_MAG_DATA_X_LSB, SCALE)
            }
    ...
    pub fn temperature(&mut self) -> Result<i8, Error<E>> {
        ...
        let temp = self.read_u8(registers::BNO055_TEMP).map_err(Error::I2c)? as i8;
        Ok(temp)
    }
    ...
    fn address(&self) -> u8 {
        if !self.use_default_addr {
            registers::BNO055_ALTERNATE_ADDR
        } else {
            registers::BNO055_DEFAULT_ADDR
        }
    }
...
impl AxisRemap {
    pub fn x(&self) -> AxisConfig {
        self.x
    }

    pub fn y(&self) -> AxisConfig {
        self.x
    }

    pub fn z(&self) -> AxisConfig {
        self.z
    }
}
...
impl AxisRemap {
    pub fn builder() -> AxisRemapBuilder {
        AxisRemapBuilder {
            remap: AxisRemap {
                x: AxisConfig::AXIS_AS_X,
                y: AxisConfig::AXIS_AS_Y,
                z: AxisConfig::AXIS_AS_Z,
            },
        }
    }
}
...
impl AxisRemapBuilder {
    pub fn swap_x_with(mut self, to: AxisConfig) -> AxisRemapBuilder {
        let old_x = self.remap.x;

        match to {
            AxisConfig::AXIS_AS_X => self.remap.x = old_x,
            AxisConfig::AXIS_AS_Y => self.remap.y = old_x,
            AxisConfig::AXIS_AS_Z => self.remap.z = old_x,
    ...
    pub fn swap_y_with(mut self, to: AxisConfig) -> AxisRemapBuilder {
        let old_y = self.remap.y;

        match to {
            AxisConfig::AXIS_AS_X => self.remap.x = old_y,
            AxisConfig::AXIS_AS_Y => self.remap.y = old_y,
            AxisConfig::AXIS_AS_Z => self.remap.z = old_y,
    ...
    pub fn swap_z_with(mut self, to: AxisConfig) -> AxisRemapBuilder {
        let old_z = self.remap.z;

        match to {
            AxisConfig::AXIS_AS_X => self.remap.x = old_z,
            AxisConfig::AXIS_AS_Y => self.remap.y = old_z,
            AxisConfig::AXIS_AS_Z => self.remap.z = old_z,
...
pub const BNO055_CALIB_SIZE: usize = core::mem::size_of::<CalibrationProfile>();

impl CalibrationProfile {
    pub fn from_buf(buf: &[u8; BNO055_CALIB_SIZE]) -> CalibrationProfile {
    ...
    pub fn as_bytes(&self) -> &[u8] {
        unsafe {
            core::slice::from_raw_parts(
                ...
                ::core::mem::size_of::<CalibrationProfile>(),
            )
        }
    }
}
