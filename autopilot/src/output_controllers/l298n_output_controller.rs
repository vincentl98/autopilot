use crate::traits::OutputController;
use rppal::{
...
impl Motor {
    pub fn new(pins: MotorPins) -> Self {
        ...
        Motor {
            ...
            pwm: rppal::pwm::Pwm::with_frequency(
                pins.pwm_channel,
                50.0,
                0.,
                Polarity::Normal,
            ...
            .expect(format!("Failed to open PWM channel {:?}", pins.pwm_channel).as_str()),
        }
    }
...
impl OutputController<(f64, f64)> for L298NOutputController {
    fn write_output(&mut self, output: (f64, f64)) -> Result<(), Box<dyn Error>> {
